...
{   int epoll_fd;              // Epoll instance FD
    ...
    long long total_rtt;       // Total RTT (in microseconds) for successful messages
    long total_messages;       // Number of messages that got a valid echo
...
/*
 * Client thread function - no retransmission
 */
void *client_thread_func(void *arg) 
{
    ...
    // For each request, send one packet and wait for a response
    ...
    {   
        ...
        {   // Timeout - No response received - Count as lost
            fprintf(stderr, "Timeout on client %d, seq %d. Packet lost.\n",
        ...
        for (int j = 0; j < nready; j++) 
        {
            if (events[j].data.fd == data->socket_fd) 
            {
                ...
                // Validate that the received packet matches what was sent
    ...
    // Print metrics for client thread and close
...
void run_client() 
{
    ...
    // Clean up sockets and epoll descriptors
    for (int i = 0; i < num_client_threads; i++) 
...
/*
 * run_server:
 * Creates a UDP socket, binds to the server address, and uses epoll
 * in a run-to-completion loop to receive packets and echo them back.
 */
void run_server() 
{
